var PixelWind = (() => {
  ...
  // lib/index.ts
  var PixelWind = class _PixelWind {
    // client-only
    readAsElement(img) {
    ...
    async readAsDataUrl(url) {
      ...
      try {
        const mat = await _PixelWind.resolveWithUrl(url);
      ...
      switch (mode) {
        ...
        case this.RESIZE.INTER_LINEAR:
          execMat.recycle((_pixel, row, col) => {
            ...
            const NR = _PixelWind.calcResizeLinerFunc(
              R_x1_y1,
              R_x2_y1,
              R_x1_y2,
              R_x2_y2,
            ...
            const NG = _PixelWind.calcResizeLinerFunc(
              G_x1_y1,
              G_x2_y1,
              G_x1_y2,
              G_x2_y2,
            ...
            const NB = _PixelWind.calcResizeLinerFunc(
              B_x1_y1,
              B_x2_y1,
              B_x1_y2,
              B_x2_y2,
            ...
            const NA = _PixelWind.calcResizeLinerFunc(
              A_x1_y1,
              A_x2_y1,
              A_x1_y2,
              A_x2_y2,
    ...
    gray(mat) {
      mat.recycle((pixel, row, col) => {
        const [R, G, B] = pixel;
        const Gray = Math.floor(_PixelWind.rgbToGray(R, G, B));
    ...
    async gaussianBlur(mat, ksize, sigmaX = 0, sigmaY = sigmaX) {
      ...
      const gaussianKernel = _PixelWind.calcGaussianKernel(ksize, sigmaX, sigmaY);
    ...
    LUT(mat, lutTable) {
      if (arguments.length === 1 || !lutTable?.length) {
        ...
        for (let i = 0; i < 256; i++) {
          lutTable[i] = Math.min(
            255,
            Math.floor(i * _PixelWind.SATURATION_CONTRAST)
          );
        }
      }
      mat.recycle((pixel, row, col) => {
    ...
    threshold(mat, threshold, maxValue, type = this.THRESHOLD_TYPE.BINARY, mode = this.THRESHOLD_MODE.THRESHOLD) {
      mat.recycle((_pixel, row, col) => {
        ...
        const gray = _PixelWind.rgbToGray(R, G, B);
        let newValue;
        switch (mode) {
          case this.THRESHOLD_MODE.THRESHOLD:
            newValue = _PixelWind.calcThresholdValue(
              gray,
              threshold,
              maxValue,
              type
          ...
          case this.THRESHOLD_MODE.OTSU:
            newValue = _PixelWind.calcThresholdValue(
              gray,
              _PixelWind.calcOtsuThreshold(mat),
              maxValue,
              type
            );
            break;
    ...
    static rgbToGray(R, G, B) {
      return R * _PixelWind.GRAY_SCALE_RED + G * _PixelWind.GRAY_SCALE_GREEN + B * _PixelWind.GRAY_SCALE_BLUE;
    }
    // 用于解析url图片
    ...
    static calcGaussianKernel(ksize, sigmaX, sigmaY) {
      ...
      for (let x = -half; x <= half; x++) {
        ...
        for (let y = -half; y <= half; y++) {
          const col = half + y;
          const gaussianFunctionRes = _PixelWind.gaussianFunction(
            x,
            y,
            sigmaX,
            sigmaY
