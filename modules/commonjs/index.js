...
class PixelWind {
  ...
  readAsElement(img) {
    ...
    return new import_mat.Mat(imageData);
  }
  // base64 或者非跨域url
  async readAsDataUrl(url) {
    if (!url) {
      (0, import_log.errorlog)("no url\uFF01");
    }
    try {
  ...
  async readAsData(blob) {
    if (!blob.size) {
      (0, import_log.errorlog)("no content blob");
    }
  ...
  clip(mat, x, y, width, height) {
    ...
    const newMat = new import_mat.Mat(
  ...
  resize(mat, scaleWidth, scaleHeight, mode = this.RESIZE.INTER_LINEAR) {
    ...
    const execMat = new import_mat.Mat(imageData);
    const {
      size: { width, height }
    } = mat;
  ...
  medianBlur(mat, size) {
    if (size % 2 !== 1) {
      (0, import_log.errorlog)("size\u9700\u4E3A\u5947\u6574\u6570\uFF01");
    }
  ...
  groundGlassFilter(mat, offset = 5, bothFamily = true) {
    if (!offset || offset <= 0) {
      (0, import_log.errorlog)("offset \u9700\u4E3A\u6B63\u6574\u6570\uFF01");
    }
    const { rows, cols } = mat;
  ...
  static resolveWithUrl(url) {
    return new Promise((resolve, reject) => {
      ...
      img.addEventListener("load", () => {
        ...
        resolve(new import_mat.Mat(imageData));
        img.remove();
        cavans.remove();
      });
